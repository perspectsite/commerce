...
const Product = ({ product_categories_json }) => {
  ...
  useEffect(() => {
    if (product_categories_json && id && cat_id) {
      const currentProductCategory = product_categories_json.filter(
        (ct) => parseInt(ct.category_id) === parseInt(cat_id)
      );
      if (currentProductCategory && currentProductCategory.length) {
        const currentCategoryProducts = currentProductCategory[0].products;
        const currentProduct = currentCategoryProducts.filter((item) => {
          return parseInt(item.product_id) === parseInt(id) ? item : "";
        });
  ...
  }, [product_categories_json, id, cat_id]);
            ...
            singleProductData.map((items, idx) => (
              <div key={idx} className="pl-5">
                ...
                <div className="border-t py-4">
                  ...
                  <p className="mb-3">{items.description.replace("<p>", "")}</p>
                </div>
              </div>
            ))}
        </div>
